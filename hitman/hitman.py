...
complete_map_example = {
    ...
    (3, 2): HC.GUARD_E,
    (4, 2): HC.EMPTY,
    (5, 2): HC.CIVIL_W, #HC.CIVIL_E,
    (6, 2): HC.CIVIL_E, #HC.CIVIL_W,
    (0, 1): HC.EMPTY,
    (1, 1): HC.EMPTY,
    (2, 1): HC.EMPTY,
    (3, 1): HC.EMPTY, #TODO HC.GUARD_E,
    (4, 1): HC.EMPTY,
    (5, 1): HC.EMPTY,
    (6, 1): HC.EMPTY,
    (0, 0): HC.EMPTY,
...
class HitmanReferee:
    def __init__(self, filename: str = ""):
        ...
        self.__phase1_guess_score = 0 #score de reconnaissance
        self.__phase2_penalties = 0
        self.__pos = (0, 0)
        self.__orientation = HC.N
        self.__has_guessed = False # a deviné
        self.__is_in_guard_range = False # est dans la vision d'un garde
        self.__is_in_civil_range = False # est dans la vision d'un civil
        self.__phase1_history: List[str] = [] # historique des coups joués en phase 1
        self.__phase2_history: List[str] = [] # historique des coups joués en phase 2
        self.__has_suit = False # a la veste
        self.__suit_on = False # a mis la veste
        self.__has_weapon = False # a l'arme
        self.__is_target_down = False # a abattu la cible
    ...
    def __get_status_phase_1(self, err: str = "OK"):
        return {
            "status": err, # = OK
            "phase": self.__phase, # = 1
            "guard_count": self.__guard_count,
            "civil_count": self.__civil_count,
            "m": self.__m,
            "n": self.__n,
            "position": self.__pos, # = (0, 0)
            "orientation": self.__orientation, # = HC.N
            "vision": self.__get_vision(), # vision de Hitman à sa position (0, 0)
            "hear": self.__get_listening(), # périmètre = 2 autour du perso (diagonale incluse)
            "penalties": self.__phase1_penalties, # = 0 (compteur d'actions de Hitman + 1 à chaque déplacement)
            "is_in_guard_range": self.__is_in_guard_range, # = False (passe à True si Hitman passe devant un garde)
        }
    ...
    def __get_world_content(self, x: int, y: int):
        # provisoire
        return self.__world[self.__m - y - 1][x] #vision de la valeur de la case en renseignant x et y

    def __update_world_content(self, x: int, y: int, new_content: HC): #TODO comprendre
    ...
    def __get_vision(self, dist=3):
        ...
        pos = self.__pos
        x, y = pos
        vision = [] #TODO modification : vision = [(pos, self.__get_world_content(x, y))]
